 ...
 * @retval USB_OTG_STS : status
 */
USB_OTG_STS USB_OTG_WritePacket(USB_OTG_CORE_HANDLE *pdev, uint8_t *src,
 ...
 * @retval None
 */
void *USB_OTG_ReadPacket(USB_OTG_CORE_HANDLE *pdev, uint8_t *dest,
		uint16_t len) {
	uint32_t i = 0;
...
USB_OTG_STS USB_OTG_CoreInitHost(USB_OTG_CORE_HANDLE *pdev) {
	...
	if (pdev->cfg.coreID == USB_OTG_HS_CORE_ID) {
		...
		USB_OTG_WRITE_REG32(&pdev->regs.GREGS->DIEPTXF0_HNPTXFSIZ,
				nptxfifosize.d32);
...
USB_OTG_STS USB_OTG_EnableHostInt(USB_OTG_CORE_HANDLE *pdev) {
	...
	// Xavier XH : I don't want this one...
	// intmsk.b.sofintr = 1;
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev, uint8_t hc_num) {
	...
	if (pdev->host.hc[hc_num].xfer_len > 0) {
		num_packets = (pdev->host.hc[hc_num].xfer_len
				+ pdev->host.hc[hc_num].max_packet - 1)
	...
	if (pdev->cfg.dma_enable == 1) {
		USB_OTG_WRITE_REG32(&pdev->regs.HC_REGS[hc_num]->HCDMA,
				(unsigned int)pdev->host.hc[hc_num].xfer_buff);
	}
		...
		if ((pdev->host.hc[hc_num].ep_is_in == 0)
			...
			case EP_TYPE_BULK:
				...
				if (len_words > hnptxsts.b.nptxfspcavail) {
					...
					USB_OTG_MODIFY_REG32( &pdev->regs.GREGS->GINTMSK, 0,
							intmsk.d32);
				}
			...
			USB_OTG_WritePacket(pdev, pdev->host.hc[hc_num].xfer_buff, hc_num,
